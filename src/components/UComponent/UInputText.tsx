import { useState } from "react";
export const UInputText = ({
  name,
  onChange,
  value,
  error,
  placeholder,
}: {
  name: string;
  onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
  value: string;
  error: string | undefined;
  placeholder: string;
}) => {
  const [showPassword, setShowPassword] = useState("password");

  const passwordToggle = () => {
    if (showPassword === "password") {
      setShowPassword("text");
    } else {
      setShowPassword("password");
    }
  };

  if (name === "confirmPassword") {
    return (
      <div className="relative">
        <div className="relative flex items-center">
          <input
            className="w-full  bg-neutral-900 text-white border border-gray-700 px-4 py-2 rounded-lg resize-none"
            type={showPassword}
            placeholder={placeholder}
            name={name}
            onChange={onChange}
            value={value}
          />

          <button
            type="button"
            className="absolute w-8 h-8 right-1 cursor-pointer hover:bg-gray-700 rounded-full"
            onClick={passwordToggle}
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <path
                  opacity="0.5"
                  d="M2 12C2 13.6394 2.42496 14.1915 3.27489 15.2957C4.97196 17.5004 7.81811 20 12 20C16.1819 20 19.028 17.5004 20.7251 15.2957C21.575 14.1915 22 13.6394 22 12C22 10.3606 21.575 9.80853 20.7251 8.70433C19.028 6.49956 16.1819 4 12 4C7.81811 4 4.97196 6.49956 3.27489 8.70433C2.42496 9.80853 2 10.3606 2 12Z"
                  fill="#1C274C"
                  className="hover:fill-white transition-colors duration-200"
                ></path>
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M8.25 12C8.25 9.92893 9.92893 8.25 12 8.25C14.0711 8.25 15.75 9.92893 15.75 12C15.75 14.0711 14.0711 15.75 12 15.75C9.92893 15.75 8.25 14.0711 8.25 12ZM9.75 12C9.75 10.7574 10.7574 9.75 12 9.75C13.2426 9.75 14.25 10.7574 14.25 12C14.25 13.2426 13.2426 14.25 12 14.25C10.7574 14.25 9.75 13.2426 9.75 12Z"
                  fill="#1C274C"
                ></path>
              </g>
            </svg>
          </button>
        </div>
        <p className="absolute text-red-600">{error}</p>
      </div>
    );
  }

  if (name === "password") {
    return (
      <div className="relative">
        <div className="relative flex items-center">
          <input
            className="w-full  bg-neutral-900 text-white border border-gray-700 px-4 py-2 rounded-lg resize-none"
            type={showPassword}
            placeholder={placeholder}
            name={name}
            onChange={onChange}
            value={value}
          />

          <button
            type="button"
            className="absolute w-8 h-8 right-1 cursor-pointer hover:bg-gray-700 rounded-full"
            onClick={passwordToggle}
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <path
                  opacity="0.5"
                  d="M2 12C2 13.6394 2.42496 14.1915 3.27489 15.2957C4.97196 17.5004 7.81811 20 12 20C16.1819 20 19.028 17.5004 20.7251 15.2957C21.575 14.1915 22 13.6394 22 12C22 10.3606 21.575 9.80853 20.7251 8.70433C19.028 6.49956 16.1819 4 12 4C7.81811 4 4.97196 6.49956 3.27489 8.70433C2.42496 9.80853 2 10.3606 2 12Z"
                  fill="#1C274C"
                  className="hover:fill-white transition-colors duration-200"
                ></path>
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M8.25 12C8.25 9.92893 9.92893 8.25 12 8.25C14.0711 8.25 15.75 9.92893 15.75 12C15.75 14.0711 14.0711 15.75 12 15.75C9.92893 15.75 8.25 14.0711 8.25 12ZM9.75 12C9.75 10.7574 10.7574 9.75 12 9.75C13.2426 9.75 14.25 10.7574 14.25 12C14.25 13.2426 13.2426 14.25 12 14.25C10.7574 14.25 9.75 13.2426 9.75 12Z"
                  fill="#1C274C"
                ></path>
              </g>
            </svg>
          </button>
        </div>
        <p className="absolute text-red-600">{error}</p>
      </div>
    );
  }
  return (
    <div className="relative">
      <input
        className="w-full  bg-neutral-900 text-white border border-gray-700 px-4 py-2 rounded-lg resize-none"
        type="text"
        placeholder={placeholder}
        name={name}
        onChange={onChange}
        value={value}
      />
      <p className="absolute text-red-600">{error}</p>
    </div>
  );
};
